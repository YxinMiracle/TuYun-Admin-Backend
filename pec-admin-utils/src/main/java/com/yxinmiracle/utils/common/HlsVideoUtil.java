...
public class HlsVideoUtil extends  VideoUtil {
    ...
    String previewPictureFolderPath;
    String fileId;
    public HlsVideoUtil(String ffmpeg_path, String video_path, String m3u8_name,String m3u8folder_path,String previewPictureFolderPath,String fileId){
        super(ffmpeg_path);
        this.ffmpeg_path = ffmpeg_path;
        this.video_path = video_path;
        this.m3u8_name = m3u8_name;
        this.m3u8folder_path = m3u8folder_path;
        this.previewPictureFolderPath = previewPictureFolderPath;
        this.fileId = fileId;
    }
    ...
    private void clearpreviewPicture(String previewPictureFolderPath){
        File previewPictureDir = new File(previewPictureFolderPath);
        if(!previewPictureDir.exists()){
            previewPictureDir.mkdirs();
        }
    }
...
//        commend.add("D:/BaiduNetdiskDownload/Movies/test1/test1.m3u8");
        ...
        String videoPreviewPictureStatus = getVideoPreviewPicture();
        if (!videoPreviewPictureStatus.equals("success")){
            return videoPreviewPictureStatus;
        }
        return "success";
    }

    public String getVideoPreviewPicture(){
        clearpreviewPicture(previewPictureFolderPath);
        // ffmpeg -i b.mp4 -vf "select=eq(pict_type\,I)" -vsync vfr -qscale:v 2 -f image2 D:/test/%03d.jpg
        List<String> commend = new ArrayList<String>();
        commend.add(ffmpeg_path);
        commend.add("-i");
        commend.add(video_path);
        ...
        commend.add("-vsync");
        commend.add("vfr");
        commend.add("-qscale:v");
        commend.add("2");
        commend.add("-s");
        commend.add("160*90");
        commend.add("-f");
        commend.add("image2");
        commend.add(previewPictureFolderPath + fileId + "_%03d.jpg");
        String outstring = null;
        try {
            ProcessBuilder builder = new ProcessBuilder();
            builder.command(commend);
